...
public class EmojiEditText extends AppCompatEditText {
    ...
    public boolean onKeyPreIme(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.KEYCODE_SOFT_LEFT) {
            if (null != mOnPressBackListener) {
                mOnPressBackListener.onPressBack();
            }
        }
        return super.onKeyPreIme(keyCode, event);
    }

    public void setOnPressBackListener(OnPressBackListener listener) {
        mOnPressBackListener = listener;
    }
