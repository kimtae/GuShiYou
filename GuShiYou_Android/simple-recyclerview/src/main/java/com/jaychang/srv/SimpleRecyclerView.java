...
public class SimpleRecyclerView extends RecyclerView
    ...
    private boolean isLoadMoreToBottom;
    private boolean isLoadingMore;
    private boolean isLoadMoreViewShown;

    private int maxFlingVelocityX = Integer.MAX_VALUE;
    private int maxFlingVelocityY = Integer.MAX_VALUE;
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyle) {
        ...
        gridSpanCount = typedArray.getInt(R.styleable.srv_SimpleRecyclerView_srv_gridSpanCount, 1);
        ...
        dividerBackground = typedArray.getColor(R.styleable.srv_SimpleRecyclerView_srv_dividerBackground, Color.WHITE);
    ...
    public void setSupportsChangeAnimations(boolean supportsChangeAnimations) {
        ...
        if (null != animator) {
            animator.setSupportsChangeAnimations(supportsChangeAnimations);
        }
    }
...
//        });
    ...
    private void checkLoadMoreThreshold() {
        ...
        if (isLoadMoreToBottom && !isScrollUp) {
    ...
    private void handleLoadMoreTop() {
        onLoadMoreListener.onLoadMoreTop(this);
    }

    private void handleLoadMoreBottom() {
        onLoadMoreListener.onLoadMoreBottom(this);
    }
    ...
    private void addDividerItemDecoration(@ColorInt int color, int orientation,
        ...
        if (color != 0) {
            ...
            backgroundDrawable.setIntrinsicHeight(Utils.dpToPx(getContext(), 0.5f));
            backgroundDrawable.setIntrinsicWidth(Utils.dpToPx(getContext(), 0.5f));
            backgroundDrawable.getPaint().setColor(dividerBackground);
            drawables[0] = backgroundDrawable;

            ShapeDrawable dividerDrawable = new ShapeDrawable(new RectShape());
            dividerDrawable.setIntrinsicHeight(Utils.dpToPx(getContext(), 0.5f));
            dividerDrawable.setIntrinsicWidth(Utils.dpToPx(getContext(), 0.5f));
            dividerDrawable.getPaint().setColor(color);
            drawables[1] = dividerDrawable;

            LayerDrawable layer = new LayerDrawable(drawables);
            layer.setLayerInset(0, 0, 0, 0, 0);
            layer.setLayerInset(1, paddingLeft, paddingTop, paddingRight, paddingBottom);
            decor.setDrawable(layer);
        }
    ...
    public void showEmptyStateView() {
        ...
        if (!adapter.getAllCells().contains(emptyStateViewCell)) {
...
//            @Override
//            public void run() {
        adapter.removeCellNotNotifyDataSetChanged(loadMoreViewCell);
//            }
//        });
    ...
    public void setLoadMoreToBottom(boolean isLoadMoreForBottom) {
        this.isLoadMoreToBottom = isLoadMoreForBottom;
    }
    ...
    public boolean isLoadMoreToBottom() {
        return isLoadMoreToBottom;
    }
    ...
    public void updateCell(SimpleCell cell, Object payload) {
        adapter.updateCell(cell, payload);
    }
    ...
    public List<SimpleCell> getAllCells() {
        List<SimpleCell> normalCells = new ArrayList<>(50);
        for (SimpleCell simpleCell : adapter.getAllCells()) {
            if (!(simpleCell instanceof InternalEmptyStateViewCell) && !(simpleCell instanceof InternalLoadMoreViewCell)) {
                normalCells.add(simpleCell);
            }
        }
        return normalCells;
    }
    ...
    public void setItemViewCache(SimpleAdapter.ItemViewCache itemViewCache) {
        adapter.setItemViewCache(itemViewCache);
    }

    @Override
    public boolean fling(int velocityX, int velocityY) {
        if (velocityY > maxFlingVelocityY) {
            velocityY = maxFlingVelocityY;
        }
        if (velocityY < -maxFlingVelocityY) {
            velocityY = -maxFlingVelocityY;
        }
        if (velocityX > maxFlingVelocityX) {
            velocityX = maxFlingVelocityX;
        }
        if (velocityX < -maxFlingVelocityX) {
            velocityX = -maxFlingVelocityX;
        }
    ...
    public void setMaxFlingVelocityX(int maxFlingVelocityX) {
        this.maxFlingVelocityX = maxFlingVelocityX;
    }

    public void setMaxFlingVelocityY(int maxFlingVelocityY) {
        this.maxFlingVelocityY = maxFlingVelocityY;
    }
}
